...
kinova Cube Pick
----------------
Use Jacobian matrix and inverse kinematics control of kinova robot to pick up a box.
...
args = gymutil.parse_arguments(
    description="Kinova Jacobian Inverse Kinematics (IK) Nut-Bolt Screwing",
...
# load kinova asset
kinova_asset_file = "urdf/kortex_description/robots/kinova_gen3.urdf"
asset_options = gymapi.AssetOptions()
...
kinova_mids = 0.3 * (kinova_upper_limits + kinova_lower_limits)

# use position drive for all dofs
kinova_dof_props["driveMode"][:7].fill(gymapi.DOF_MODE_POS)
kinova_dof_props["stiffness"][:7].fill(400.0)
kinova_dof_props["damping"][:7].fill(40.0)
# grippers
kinova_dof_props["driveMode"][7:].fill(gymapi.DOF_MODE_POS)
kinova_dof_props["stiffness"][7:].fill(800.0)
kinova_dof_props["damping"][7:].fill(40.0)

# default dof states and position targets
kinova_num_dofs = gym.get_asset_dof_count(kinova_asset)
default_dof_pos = np.zeros(kinova_num_dofs, dtype=np.float32)
default_dof_pos[:7] = kinova_mids[:7]
# grippers open
default_dof_pos[7:] = kinova_upper_limits[7:]
...
for i in range(num_envs):
    ...
    # add kinova
    kinova_handle = gym.create_actor(env, kinova_asset, kinova_pose, "kinova", i, 0)
